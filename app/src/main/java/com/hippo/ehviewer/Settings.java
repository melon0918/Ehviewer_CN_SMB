...
//        }
     ...
     ****** SMB Remote Storage
     ********************/
    public static final String KEY_SMB_ENABLED = "smb_enabled";
    ...
    private static SharedPreferences getSmbSecurePrefs() {
        if (sSmbSecurePrefs == null && sContext != null) {
            try {
                MasterKey masterKey = new MasterKey.Builder(sContext)
                        .setKeyScheme(MasterKey.KeyScheme.AES256_GCM)
                        .build();
                sSmbSecurePrefs = EncryptedSharedPreferences.create(
                        sContext,
                        SMB_SECURE_PREFS_NAME,
                        masterKey,
                        EncryptedSharedPreferences.PrefKeyEncryptionScheme.AES256_SIV,
                        EncryptedSharedPreferences.PrefValueEncryptionScheme.AES256_GCM
                );
            } catch (Exception e) {
                Log.w(TAG, "Failed to create encrypted SMB prefs", e);
            }
        }
        return sSmbSecurePrefs;
    }

    public static boolean getSmbEnabled() {
        return getBoolean(KEY_SMB_ENABLED, false);
    }

    public static void setSmbEnabled(boolean enabled) {
        putBoolean(KEY_SMB_ENABLED, enabled);
    }

    @Nullable
    public static String getSmbHost() {
        return getString(KEY_SMB_HOST, null);
    }

    public static void setSmbHost(@Nullable String host) {
        putString(KEY_SMB_HOST, host);
    }

    @Nullable
    public static String getSmbShare() {
        return getString(KEY_SMB_SHARE, null);
    }

    public static void setSmbShare(@Nullable String share) {
        putString(KEY_SMB_SHARE, share);
    }

    @NonNull
    public static String getSmbPath() {
        String path = getString(KEY_SMB_PATH, "/");
        return path != null ? path : "/";
    }

    public static void setSmbPath(@NonNull String path) {
        putString(KEY_SMB_PATH, path);
    }

    @NonNull
    public static String getSmbUsername() {
        String username = getString(KEY_SMB_USERNAME, "");
        return username != null ? username : "";
    }

    public static void setSmbUsername(@NonNull String username) {
    ...
    public static void setSmbPassword(@NonNull String password) {
        SharedPreferences prefs = getSmbSecurePrefs();
        if (prefs != null) {
